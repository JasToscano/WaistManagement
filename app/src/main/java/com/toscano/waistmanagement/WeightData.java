...
public class WeightData extends SQLiteOpenHelper {
    ...
    private static WeightData _weightDB;


    private WeightData(Context context){
        super(context, DATABASE_NAME, null, VERSION);
    }

    //singleton
    public static WeightData getInstance(Context context) {
        if (_weightDB == null) {
            _weightDB = new WeightData(context);
        }
        return _weightDB;
    }
    ...
    public List<WeightClass> getAllWeights(UserModel _user) throws ParseException {
        ...
        if(cursor.moveToFirst()){
            do {
                ...
                //if the username matches then start the loop
                if (username.equals(_user.getUserName())){
